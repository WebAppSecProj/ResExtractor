...
class Cordova(BaseModule):
    def doSigCheck(self):
        if self.host_os == "android":
            ...
            Cordova_framework = os.path.join("Frameworks","Cordova.framework")
            if self._check_file_exists(Cordova_framework,is_dir=True):
                return True
            return False
        return False
    ...
    def doExtractiOS(self, working_folder):
        ...
        public_folder = os.path.join(tmp_folder,self._ipa_app_path(),"public")

        launch_path = ""

        if os.path.exists(os.path.join(public_folder,"index.html")):
            launch_path = os.path.join(extract_folder,"index.html")
        
        shutil.copytree(public_folder, extract_folder, dirs_exist_ok=True)
