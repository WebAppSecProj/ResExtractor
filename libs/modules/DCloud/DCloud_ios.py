...
class DCloud(BaseModule):
    def doSigCheck(self):
        if self.host_os == "android":
            ...
            main_infoplist_content = self._dir_info_plist_from_ipa(self._ipa_app_path())
            #print(self.main_infoplist_content)
            if main_infoplist_content["CFBundleIdentifier"].startswith("io.dcloud"):
    ...
    def doExtractiOS(self, working_folder): 
        ...
        root_file_path = os.path.join(tmp_folder,self._ipa_app_path())

        control_file = os.path.join(root_file_path,"control.xml")
        if os.path.exists(control_file) == False:
            log.error("no control.xml in {}".format(control_file))
            return "",""
        t = ET.ElementTree(file=control_file)
        appid = ""
        for elem in t.iter(tag='app'):
            appid = elem.attrib['appid']
        
        if appid == "":
            log.error("no appid in control.xml")
            return "",""
        
        resource_path = os.path.join(root_file_path,"Pandora","apps",appid,"www")

        for dirpath, dirs, files in os.walk(resource_path):
            for tmp_file in files:
                tmp_file_path = os.path.join(dirpath,tmp_file)
                rel_path = tmp_file_path.split(resource_path)[1]
                if rel_path.startswith("/"):
                    rel_path = rel_path[1:]
                target_path = os.path.join(extract_folder,rel_path)
        ...
        manifest_path = os.path.join(extract_folder,"manifest.json")
        try:
            j = json.load(open(manifest_path,"r"))
            launch_path = j['launch_path']
        except:
            log.error("wrong json file : {}".format(manifest_path))
