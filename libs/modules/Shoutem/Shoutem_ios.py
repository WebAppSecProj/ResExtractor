...
class Shoutem(BaseModule):
    ...
    def doSigCheck(self):
        if self.host_os == "android":
            ...
            if self._dir_info_plist_from_ipa(self._ipa_app_path())["CFBundleIdentifier"].startswith("hr.apps."):
                return True
            return False
        return False

    def checkNeedCopy(self, file_name):
    ...
    def doExtractiOS(self,working_folder):
        ...
        assets_path = os.path.join(ipa_root_path,"assets")
        if os.path.exists(assets_path) == False:
            log.error("no assets dir in ipa")
            return "",""
        


        for dirpath, dirs, files in os.walk(assets_path):
            for tmp_file in files:
                if self.checkNeedCopy(tmp_file) == False:
                    continue
                file_path = os.path.join(dirpath,tmp_file)
                #print(file_path)
                rel_path = file_path.split(ipa_root_path)[1]
                if rel_path.startswith("/"):
                    rel_path = rel_path[1:]
                target_file_path = os.path.join(extract_folder,rel_path)
                #print(target_file_path)
                if not os.access(os.path.dirname(target_file_path), os.R_OK):
