...
class Biznessapps(BaseModule):
    def doSigCheck(self):
        if self.host_os == "android":
            ...
            ipa_root_path = self._ipa_app_path()
            if self._check_file_exists("BASharingTextViewController.nib"):
                return True
            if self._check_file_exists("BAResellerAppCell.nib"):
                return True
            if self._check_file_exists("BAResetPasswordViewController.nib"):
                return True
            if self._check_file_exists("BAUserLoginViewController.nib"):
                return True
            return False
        return False
    ...
    def doExtractiOS(self,working_folder):
        ...
        app_code = self._dir_info_plist_from_ipa(self._ipa_app_path())["CFBundleName"]
        if app_code !="":
            url = "https://www.cdnstabletransit.com/iphone/1.1.1/init.php?app_code="+app_code+"&ba_version=50&firstRun=0&notFirstLaunch=1&device_user_id=eaae73dd691273c3&device=iphone5"
            print(url)
            res = requests.post(url=url) #res即返回的响应对象
            # 3. 解析响应
            result=res.content
            result_json_file = open(os.path.join(extract_folder,"content.json"),"wb")
            result_json_file.write(result)
            result_json_file.close()
        
        self._dump_info(extract_folder, "")
        # clean env
