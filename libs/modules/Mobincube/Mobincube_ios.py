...
class Mobincube(BaseModule):
    ...
    def doSigCheck(self):
        if self.host_os == "android":
            ...
            if self._dir_info_plist_from_ipa(self._ipa_app_path())["CFBundleIdentifier"].startswith("com.mobincube.ios"):
                return True
            return False
        return False
    ...
    def needExtract(self, file_name):
    ...
    def doExtractiOS(self,working_folder):
        ...
        source_dir = os.path.join(tmp_folder,self._ipa_app_path(),"CaponateData")

        for dirpath, dirs, files in os.walk(source_dir):
            for tmp_file in files:
                if(self.needExtract(tmp_file)):
                    source_file = os.path.join(dirpath,tmp_file)
                    tmp_path = source_file.split(source_dir)[1]
                    if tmp_path.startswith("/"):
                        tmp_path = tmp_path[1:]
                    target_file = os.path.join(extract_folder,tmp_path)
